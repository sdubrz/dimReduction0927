...
def main_run(main_path, data_name, nbrs_k=30, yita=0.1, method_k=30, max_eigen_numbers=5, method="MDS",
    ...
    if method == "newMDS":  # 采用合并的降维方式
        ...
        #                                                        yita=yita, save_path=save_path, weighted=weighted,
        #                                                        label=label, y_precomputed=y_precomputed)
        y, y_list_add, y_list_sub = TSNE_Perturb.perturb_tsne_one_by_one(x, nbrs_k=nbrs_k, y_init=y_random,
                                                                       method_k=method_k,
    ...
    elif method == "cTSNEPlus":
        from Perturb2020 import TSNE_PerturbPlus
        y, y_list_add, y_list_sub = TSNE_PerturbPlus.perturb_tsne_one_by_one(x, nbrs_k=nbrs_k, y_init=y_random,
                                                                       method_k=method_k,
                                                                       MAX_EIGEN_COUNT=max_eigen_numbers,
                                                                       method_name=method,
                                                                       yita=yita, save_path=save_path,
        ...
        y, y_list_add, y_list_sub = MDS_Perturb.perturb_mds_one_by_one(x, nbrs_k=nbrs_k, y_init=y_random, method_k=method_k,
                                                                   MAX_EIGEN_COUNT=max_eigen_numbers,
                                                                   method_name=method,
                                                                   yita=yita, save_path=save_path, weighted=weighted,
                                                                   label=label, y_precomputed=y_precomputed, local_struct=local_structure)
...
def run_test(data_name0=None):
    ...
    # 计算KNN相似性
    # VisualizationKNN.KNN_similar(last_path)  # 太浪费时间，暂时注释掉
