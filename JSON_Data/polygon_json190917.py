...
def merge_json(main_path, data_name, method_name, yita, method_k, max_k, max_eigen_numbers, proportion_threshold, adapt_threshold, group_num, draw_kind=None, MAX_EIGEN_NUMBER=4, weighted=True, MAX_NK=(0.36, 0.5)):
    ...
    angles_reader = np.loadtxt(read_path+"angle_+-sumweighted.csv", dtype=np.str, delimiter=",")  # 李代桃僵
    ...
    # linearityChange = linearity_change(eigen1_div_eigen2_project, eigen1_div_eigen2)  # 李代桃僵
    # np.savetxt(read_path+"linearityChange.csv", linearityChange, fmt="%f", delimiter=",")
    linearityChange_reader = np.loadtxt(read_path+'sin_1_2.csv', dtype=np.str, delimiter=',')
    linearityChange = linearityChange_reader.astype(np.float)
    ...
    for i in range(0, n-1):
        ...
        temp_num = linearityChange[i] + 0.0
    ...
    final_temp_num = linearityChange[n-1] + 0.0
