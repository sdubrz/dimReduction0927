...
def barycenter_weights_i(X, n_neighbors, index, reg=1e-3, n_jobs=None):
    """
    只计算一个点的权重
    :param X: 数据矩阵
    :param n_neighbors: 邻居的个数
    :param index: 要计算的点的索引号
    :param reg:
    :param n_jobs:
    :return:
    """
    ...
    X = knn._fit_X
    n_samples = X.shape[0]
    ind = knn.kneighbors(X, return_distance=False)[:, 1:]  # K近邻关系矩阵
    A = X[ind[index, :]]  # 近邻数据矩阵

    v = np.ones(n_neighbors, dtype=X.dtype)
    C = A - X[index, :]  # broadcasting
    G = np.dot(C, C.T)
    trace = np.trace(G)
    if trace > 0:
        R = reg * trace
...
def barycenter_kneighbors_graph(X, n_neighbors, reg=1e-3, n_jobs=None):
    ...
    # np.savetxt(path+"knn.csv", index, fmt='%f', delimiter=",")
    X = knn._fit_X
    n_samples = X.shape[0]
    ind = knn.kneighbors(X, return_distance=False)[:, 1:]  # K近邻关系矩阵
    # np.savetxt(path+"ind.csv", ind, fmt='%d', delimiter=",")
