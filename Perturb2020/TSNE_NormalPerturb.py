...
class TSNEPerturb:
    ...
    def init_y(self):
        time1 = time.time()
        t_sne = CTSNE_Normal.cTSNE(n_component=2, perplexity=self.n_nbrs/3.0)
...
def perturb_tsne_one_by_one(data, nbrs_k, y_init, method_k=30, MAX_EIGEN_COUNT=5, method_name="cTSNE",
    ...
    if not method_name == "cTSNE_Normal":
        print("该方法只支持 cTSNE_Normal 降维方法")
