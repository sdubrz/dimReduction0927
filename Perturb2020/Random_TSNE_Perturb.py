...
class TSNEPerturb:
    def __init__(self, X, n_nbrs, y_random):
        self.X = X
    ...
    def init_y(self):
        ...
        if self.y_random is None:
            ...
            t_sne = cTSNE.cTSNE(n_component=2, perplexity=self.n_nbrs / 3.0)
            Y = t_sne.fit_transform(self.X, max_iter=30000, y_random=self.y_random, early_exaggerate=False)
        self.Y = Y
        self.Px = t_sne.P
        self.Q = t_sne.Q
        time2 = time.time()
    ...
    def perturb(self, vectors_list, weights):
        ...
        """
        time2 = time.time()
        self.y_add_list, self.y_sub_list = RandomPerturb.random_perturb_all(self.X, self.Y, "cTSNE", vectors_list, weights, self.perplextiy)
        time3 = time.time()
...
def perturb_tsne_one_by_one(data, nbrs_k, y_init=None, method_k=30, MAX_EIGEN_COUNT=5, method_name="cTSNE",
    ...
    if not method_name == "cTSNE_random":
        print("该方法只支持 cTSNE 降维方法")

    tsne_perturb = TSNEPerturb(data, method_k, y_init)
    y = tsne_perturb.Y
    print("初次降维已经计算完毕")
