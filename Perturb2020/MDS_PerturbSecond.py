...
from Derivatives.MDS_DerivativeSecond import MDS_SecondDerivative
...
class MDSPerturb:
    ...
    def perturb(self, vectors_list, weights):
        ...
        derivative = MDS_SecondDerivative(self.X, self.Y)
        self.yHx = derivative.getH()
        self.P = derivative.J
        self.Hessian = derivative.Hessian1
        self.Jacobi = derivative.Jacobi1
        time2 = time.time()
        print("导数矩阵已经计算完成，用时为 ", time2 - time1)
        vector_perturb = VectorPerturbSecond(self.Y, self.P, self.yHx)
...
def perturb_mds_one_by_one(data, nbrs_k, y_init, method_k=30, MAX_EIGEN_COUNT=5, method_name="MDS",
    ...
    # np.savetxt(save_path0+"MDS_Hessian2.csv", np.matmul(mds_perturb.Hessian, np.linalg.pinv(mds_perturb.Hessian)), fmt='%f', delimiter=",")
