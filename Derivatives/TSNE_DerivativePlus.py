...
def derivative_matrix_memory(X, Y, Dy, beta, P0, H):
    ...
    P = -1 * P
    finish_time = time.time()
    print("计算导数矩阵(memory version)耗时 ", finish_time-begin_time)
    return P
...
def Jxy_plus(H, J):
    ...
    P = np.zeros((2 * n, m))
    for i in range(0, n):
        P[i * 2:i * 2 + 2, :] = np.matmul(H_[i * 2:i * 2 + 2, :], J[:, i * m:i * m + m])
...
class TSNE_DerivativePlus:
    def __init__(self):
        self.P = None  # 只计算原来完整的P矩阵的对角线部分
    ...
    def getP_memory(self, X, Y, P, Q, P0, beta):
        """
        计算 dY/dX更加节省内存的版本
        :param X: 高维数据矩阵
        :param Y: 降维结果矩阵
