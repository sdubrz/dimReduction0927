...
def test_file_data_3d():
    read_path = "E:\\Project\\result2019\\TPCA1008\\datasets\\manifoldcrossspace\\data.csv"
    data_reader = np.loadtxt(read_path, dtype=np.str, delimiter=',')
    X = data_reader[:, :].astype(np.float)
    print(X.shape)

    test = SMMC(X)
    test.train_mppca(d=3, M=20, max_iter=200, tol=1e-4, kmeans_init=False)
    K = 2
    locs = test.run_cluster(8, 20, K)
    ...
    ax = fig.add_subplot(111, projection='3d')
    for i in range(K):
        ax.plot(X[locs[:, i], 0], X[locs[:, i], 1], X[locs[:, i], 2], 'o')
    plt.show()
